...
impl TimelineWidget {
    pub fn new(width: i32, height: i32, length: i32) -> Rc<RefCell<TimelineWidget>> {
        ...
        TimelineWidget::create_ui(w.clone(), width);

        w
    }

    fn create_ui(self_: Rc<RefCell<TimelineWidget>>, width: i32) {
        ...
        let overlay = {
            ...
            overlay.add_overlay(&timeline.tracker);
            overlay.set_overlay_pass_through(&timeline.tracker, true);
            overlay
        };
        let vbox = gtk::Box::new(gtk::Orientation::Vertical, 0);

        overlay.add(&vbox);
        vbox.pack_start(&timeline.ruler_box, true, true, 10);
        vbox.pack_start(timeline.box_viewer.borrow().as_widget(), true, true, 0);
